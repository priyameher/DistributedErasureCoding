...
#include<sys/stat.h>
#include<fcntl.h>
#include "/home/sakshik/Desktop/Veritas/isa-l/include/erasure_code.h"

#define NUM_DATA 8
#define NUM_PARITY 3
#define BUCKETSIZE 5
#define DIRECTORY_PATH "/home/sakshik/Desktop/ObjectStorage/"
...
//TODO return boolean value for success 
...
{	
	...
	//TODO gen[d+p * d+p] and previous one was gen[d+p * d]
	unsigned char gen1[(NUM_DATA+NUM_PARITY)*(NUM_DATA+NUM_PARITY)];
	//unsigned char gen_recovery[(NUM_DATA + NUM_PARITY) * (NUM_DATA + NUM_PARITY)];
	...
	{
		//is a file
		//TODO replace with check_if_file_exists function
		succ=0;
		if(isDir(targetFilePath)==0)
		{
			...
			else
			{
				...
				for(i = 0 ; i < NUM_DATA ; ++i)
				{
					...
					{
						// TODO make function for missing folder
...
*/
				...
				for(i = 0 ; i < (NUM_PARITY) ; ++i)
				{
					temp_buffs[i] = (unsigned char *)malloc(sizeof(unsigned char)*file_size);
					memset(temp_buffs[i], 0, file_size);
				}

				ec_encode_data_base(file_size, NUM_DATA, missingChunks, r_tables, available_buffs, temp_buffs);
				...
				// TODO function for missing folder
...
int put(struct Node **arr,char file_path[100])
{
	...
	else
	{
		...
		//for data
//DONE-	//TODO In future target corner case = odd filesize
		for(i=0; i<NUM_DATA; i++)
		{
			if(i == NUM_DATA - 1){
	    			chunk_size = file_size-(chunk_size * (NUM_DATA - 1));	
...
//DONE-	//TODO Dont use static value use Macros	    	
...
//DONE-	//TODO iterate through for loop allocate memory and then proceed further
		/*Shreya Shirwadkar To Everyone
		5:14:05 PM
		SS
...
//DONE-	//TODO print the data and parity chunks i<size/8
		//TODO iterate through for loop to free data and parity chunks
//DONE-	//TODO error handling - cannot open file exit(0)
...
int putHash(struct Node **arr,int uid,char filepath[100],long int file_size){
	...
	else
	{
		...
		temp=(arr[bucket_no]);
		//TODO Replace with append first logic
