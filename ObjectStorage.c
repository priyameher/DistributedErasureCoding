...
#include<sys/stat.h>
#include<fcntl.h>
#include "/home/ubuntu/isa-l/include/erasure_code.h"
	...
	do
	{
		if(isDir(targetFilePath)==0)
		{
			...
			if(missingChunks == 0)
			{
				do
				{
					res = check_if_file_exists(targetFilePath);
			...
			*/
			...
			if(check_if_file_exists(targetFilePath) == 1)
			{
				for(i = 0 ; i < NUM_DATA ; ++i)
				{
					if(availableChunks[i] == 0 && i == NUM_DATA - 1)
					{
						missing(uid, availableChunks);
						break;
					}
	...
	else
	{
	    	...
	    	for(i=0; i<NUM_DATA; i++)
		{
			...
			if(check != 0)
			{
	    		...
	    		sprintf(file_chunk, DIRECTORY_PATH"folder%d/%d_chunk%d", i, uid, i); 
...
void missing(int uid, int ans[])
{
	...
	for(i = 0 ; i < NUM_DATA ; ++i)
	{
		sprintf(chunk_path, DIRECTORY_PATH"folder%d/%d_chunk%d%c", i, uid, i, '\0');  
